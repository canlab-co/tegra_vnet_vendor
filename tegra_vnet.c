					 ...
					 struct net_device *ndev)
{
	...
	val = dma_common_rd(tvnet->dma_base, DMA_READ_DOORBELL_OFF);
	val &= ~RD_DOORBELL_NUM_MASK;
	val |= DMA_RD_DATA_CH << RD_DOORBELL_NUM_SHIFT;
	dma_common_wr(tvnet->dma_base, val, DMA_READ_DOORBELL_OFF);
